...
class Attention_LoRA(nn.Module):
    def __init__(self, dim, rank, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, x, task_id = -1, register_hook=False, prompt=None, get_input_matrix = False):
        ...
        if task_id > -1 and self.apply_lora:
            k = k + self.lora_B_k(self.lora_A_k(x)).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
            v = v + self.lora_B_v(self.lora_A_v(x)).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.attn = Attention_LoRA(
            dim, rank = kwargs['rank'], num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
...
class ViTZoo_in21k_lora(nn.Module):
    def __init__(self, num_classes=10, pretrained=False, **kwargs):
        super(ViTZoo_in21k_lora, self).__init__()
        ...
        if pretrained:
            ...
            zoo_model = VisionTransformer(img_size=224, patch_size=16, embed_dim=768, depth=12,
                                        num_heads=12, **kwargs)
            load_dict = timm.create_model('vit_base_patch16_224_in21k', pretrained=True).state_dict()
...
def vit_pt_imnet_in21k_lora(pretrained=False, **kwargs):
    return ViTZoo_in21k_lora(pretrained = pretrained, **kwargs)
